...
namespace PaintIn3D
{
	...
	public static partial class P3dHelper
	{
			...
			if (PlayerPrefs.HasKey(saveName) == true)
			{
				...
				if (string.IsNullOrEmpty(base64) == false)
				{
					...
					if (data != null && data.Length > 0)
					{
						if (texture2D == null)
						{
							texture2D = new Texture2D(1, 1, TextureFormat.ARGB32, false, QualitySettings.activeColorSpace == ColorSpace.Linear);
						}
		...
		{
			return RenderTexture.GetTemporary(width, height, depth, format, QualitySettings.activeColorSpace == ColorSpace.Linear ? RenderTextureReadWrite.Linear : RenderTextureReadWrite.sRGB);
		}

		public static RenderTexture GetRenderTexture(int width, int height, int depth, RenderTextureFormat format, RenderTextureReadWrite colors)
		{
			return RenderTexture.GetTemporary(width, height, depth, format, colors);
		}
