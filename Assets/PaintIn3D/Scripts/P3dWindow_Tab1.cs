...
namespace PaintIn3D
{
	/// <summary>This class handles <b>Textures</b> tab for the main Paint in 3D window.</summary>
	public partial class P3dWindow : P3dEditorWindow
	{
		...
		private bool MaterialIsShared(Material material, Material newMaterial = null)
		{
			var count = 0;
		...
		private bool TextureAlreadyLocked(Texture texture)
		{
			for (var i = paintables.Count - 1; i >= 0; i--)
			{
				var paintable = paintables[i];

				for (var j = paintable.PaintableTextures.Count - 1; j >= 0; j--)
				{
					var paintableTexture = paintable.PaintableTextures[j];

					if (paintableTexture.Locked == true && paintableTexture.OldTexture == texture)
					{
						return true;
					}
				}
		...
		private P3dWindowPaintableTexture GetSharedPaintable(P3dWindowPaintableTexture target)
		{
			for (var i = paintables.Count - 1; i >= 0; i--)
			{
				var paintable = paintables[i];
				...
				for (var j = paintable.PaintableTextures.Count - 1; j >= 0; j--)
				{
					...
					if (paintableTexture == target)
					{
						return null;
					}

					if (paintableTexture.OldTexture == target.OldTexture)
					{
